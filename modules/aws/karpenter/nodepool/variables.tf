...
variable "name" {
  description = "metadata.name of the NodePool. A Kubernetes DNS-1123 label: lowercase alphanumerics and '-', starting and ending alphanumeric, at most 63 characters."
  type        = string
...
variable "ec2nodeclass_name" {
  description = "metadata.name of the EC2NodeClass this pool schedules against, rendered into spec.template.spec.nodeClassRef.name. Accepts either a literal name or the `name` output of the ec2nodeclass module; passing the module output is what orders this pool after the node class exists."
  type        = string

  validation {
    condition     = can(regex("^[a-z0-9]([-a-z0-9]*[a-z0-9])?$", var.ec2nodeclass_name)) && length(var.ec2nodeclass_name) <= 63
...
variable "lookup_ec2nodeclass" {
  description = "Read the referenced EC2NodeClass with a data source so a missing node class fails the plan instead of the apply. Set to false when the node class is created elsewhere in the same root module and therefore does not exist yet at plan time; the 2.x kubectl data source has no wait_for to absorb that."
  type        = bool
  default     = true
}

variable "requirements" {
  description = "spec.template.spec.requirements, verbatim. Caller-supplied and free-form: this module hardcodes no requirement list. `values` must be empty for Exists/DoesNotExist and a single non-negative integer for Gt/Lt/Gte/Lte."
  type = list(object({
    key       = string
    operator  = string
  ...
  validation {
    condition     = alltrue([for r in var.requirements : r.operator != "In" || length(r.values) > 0])
  ...
  validation {
    condition     = length(var.requirements) <= 100
  ...
  validation {
    condition     = alltrue([for r in var.requirements : coalesce(r.minValues, 0) <= length(r.values)])
...
variable "limits" {
  description = "spec.limits, keyed by resource name. Arbitrary resource names are accepted -- cpu, memory, nvidia.com/gpu and nodes are all valid -- so this is a map rather than a fixed cpu/memory/gpu triple. Values are Kubernetes quantities, e.g. \"500\", \"1k\", \"5000Gi\"."
  type        = map(string)
  default     = {}

  validation {
    condition = alltrue([
      for k, v in var.limits :
...
variable "taints" {
  description = "spec.template.spec.taints, verbatim. `value` is optional; `effect` comes from Kubernetes rather than Karpenter, since the CRD embeds corev1.Taint."
  type = list(object({
    key    = string
  ...
  validation {
    condition     = alltrue([for t in var.taints : length(t.key) > 0])
...
variable "labels" {
  description = "spec.template.metadata.labels. These propagate onto every node as NodeClaim requirements and therefore count toward the same 100-entry cap as `requirements`."
  type        = map(string)
  default     = {}
...
variable "weight" {
  description = "spec.weight, the pool's scheduling preference relative to other pools. Bounded 1-100. Leave null to omit it, which Karpenter treats as 0; an explicit 0 is rejected by the CRD."
  type        = number
  default     = null
...
variable "disruption" {
  description = "spec.disruption. Leave null to emit no disruption block at all: Karpenter defaults consolidationPolicy to WhenEmptyOrUnderutilized, consolidateAfter to 0s and budgets to a single {nodes: \"10%\"}, and restating those in the manifest writes server-supplied values into the caller's config."
  type = object({
...
variable "expire_after" {
  description = "spec.template.spec.expireAfter, the maximum node lifetime before Karpenter replaces it. Leave null to emit nothing and inherit the Karpenter default of 720h. Accepts s/m/h components or the literal Never."
  type        = string
  default     = null
...
variable "termination_grace_period" {
  description = "spec.template.spec.terminationGracePeriod, the bound on draining. It has no Karpenter default: left unset, a misconfigured PDB or a karpenter.sh/do-not-disrupt pod can block draining indefinitely. Unlike expire_after this does NOT accept Never."
  type        = string
  default     = null
...
variable "manifest_path" {
  description = "Path to a replacement NodePool template. Leave null to use the template bundled with this module. The path is resolved relative to the process working directory, not the module directory."
  type        = string
  default     = null
